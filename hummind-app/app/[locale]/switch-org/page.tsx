...
export default function SwitchOrgPage() {
  ...
  const [phase, setPhase] = useState<"welcome" | "organiser">("welcome");
  ...
  useEffect(() => {
    ...
    const t3 = setTimeout(() => setPhase("organiser"), 5000);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0a0a0c]">
      {phase === "welcome" && (
        ...
        >
          <span className="text-[#6e6e8a]">Bon retour </span>
          <span className="bg-gradient-to-r from-[#7C6BF5] to-[#5656a2] bg-clip-text text-transparent">
            {firstname}
          </span>
        </h1>
      )}

      {phase === "organiser" && <OrgView onDone={() => router.replace("/organisation")} />}
    </div>
  );
}

function OrgView({ onDone }: { onDone: () => void }) {
